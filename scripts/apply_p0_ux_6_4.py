...
for old, new in replacements:
    if old not in text:
        raise SystemExit(f'Expected source fragment missing: {old[:100]!r}')
    text = text.replace(old, new, 1)

path.write_text(text)

Path('frontend/test/p0_ux_6_4_rtl_accessibility_contract_test.dart').write_text(r'''import 'dart:io';
...
void main() {
  ...
  test('mobile navigation labels remain permanently visible', () {
    expect(source, contains('NavigationDestinationLabelBehavior.alwaysShow'));
    expect(
      source,
  ...
  test('add action uses localized copy', () {
    expect(source, contains('final label = AppLocalizations.of(context)!.addEntry'));
...
for temporary in [
    Path('.github/workflows/p0-ux-6-4-apply.yml'),
    Path('.github/workflows/p0-ux-6-4-pr.yml'),
    Path('scripts/apply_p0_ux_6_4.py'),
]:
