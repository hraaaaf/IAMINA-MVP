...
int _mealSearchScore(base.MealFoodItem item, String folded) {
  final values = <String>[item.fr, item.en, item.ar, ...item.aliases]
      .map(base.foldMealText)
...
List<base.MealFoodItem> searchMealFoods(String query, {int limit = 12}) {
  final folded = base.foldMealText(query);
  if (folded.length < 2) return const <base.MealFoodItem>[];
  final scored = <MapEntry<base.MealFoodItem, int>>[];
...
List<base.MealFoodItem> matchRecognizedMealFoods(Iterable<String> recognized) {
  final found = <String, base.MealFoodItem>{};
  for (final raw in recognized) {
    final needle = base.foldMealText(raw);
