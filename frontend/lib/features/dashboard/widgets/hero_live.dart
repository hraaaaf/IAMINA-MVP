...
class _HeroLive extends StatelessWidget {
  ...
  final String unit;
  final int range;
  const _HeroLive({required this.logs, required this.unit, required this.range});

  /// Calcule la tendance réelle : delta mg/dL sur 30 min entre les 2 dernières mesures.
  String? _trend() {
    if (logs.length < 2) return null;
    final a = logs[0]; final b = logs[1];
    final dt = (a.loggedAt ?? a.createdAt).difference(b.loggedAt ?? b.createdAt).inMinutes;
    if (dt == 0) return null;
  ...
  Widget build(BuildContext context) {
    ...
    final minutesAgo = latest != null
        ? DateTime.now().difference(latest.loggedAt ?? latest.createdAt).inMinutes
        : 0;
    final mealLabel  = latest?.mealType?.isNotEmpty == true ? latest!.mealType! : null;
    final insulin    = latest?.insulinUnits != null && (latest!.insulinUnits ?? 0) > 0
                  ...
                  children: [
                    Text(
                      ...
                      style: const TextStyle(
                        color: Colors.white, fontSize: 88,
                        fontWeight: FontWeight.w800, height: 0.85, letterSpacing: -4,
                      ),
                    ),
                    ...
                    Padding(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(unit, style: TextStyle(color: Colors.white.withValues(alpha: 0.72), fontSize: 14, fontWeight: FontWeight.w500)),
                          if (trend != null) ...[
                            ...
                            Container(
                              ...
                              child: Row(mainAxisSize: MainAxisSize.min, children: [
                                Icon(trendUp ? Icons.arrow_upward : Icons.arrow_downward, color: const Color(0xFFFCD34D), size: 10),
                                const SizedBox(width: 3),
                                Text(trend, style: const TextStyle(color: Color(0xFFFCD34D), fontSize: 11, fontWeight: FontWeight.w600)),
                              ]),
                            ),
                          ],
                        ],
                    ...
                    Padding(
                      ...
                      child: _AnimatedEcg(color: Colors.white.withValues(alpha: 0.75), width: 140, height: 32),
                    ),
                  ],
                ),
