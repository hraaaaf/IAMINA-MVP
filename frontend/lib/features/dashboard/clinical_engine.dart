...
class ClinicalEngine {
  ...
  static double calcTIR(List<LogEntryData> logs, double low, double high) {
    ...
    final inRange = logs
        .where((l) => l.bloodSugar >= low && l.bloodSugar <= high)
        .length;
  ...
  static double calcHigh(List<LogEntryData> logs, double high) {
    if (logs.isEmpty) return 0.0;
    return double.parse(
      ((logs.where((l) => l.bloodSugar > high && l.bloodSugar <= 250).length /
                  logs.length) *
              100)
          .toStringAsFixed(1),
    );
  ...
  static double calcVeryHigh(List<LogEntryData> logs) {
    if (logs.isEmpty) return 0.0;
    return double.parse(
      ((logs.where((l) => l.bloodSugar > 250).length / logs.length) * 100)
          .toStringAsFixed(1),
    );
  }
  ...
  static double calcLow(List<LogEntryData> logs, double low) {
    if (logs.isEmpty) return 0.0;
    return double.parse(
      ((logs.where((l) => l.bloodSugar < low && l.bloodSugar >= 54).length /
                  logs.length) *
              100)
          .toStringAsFixed(1),
    );
  ...
  static double calcVeryLow(List<LogEntryData> logs) {
    if (logs.isEmpty) return 0.0;
    return double.parse(
      ((logs.where((l) => l.bloodSugar < 54).length / logs.length) * 100)
          .toStringAsFixed(1),
    );
  }
  ...
  static double calcCV(List<LogEntryData> logs) {
    ...
    final variance =
        logs
            .map((l) => math.pow(l.bloodSugar - mean, 2))
            .reduce((a, b) => a + b) /
        (logs.length - 1);
    return double.parse(
      ((math.sqrt(variance) / mean) * 100).toStringAsFixed(1),
    );
  }
