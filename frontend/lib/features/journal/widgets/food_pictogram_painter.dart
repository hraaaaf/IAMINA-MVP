...
class FoodPictogramPainter extends CustomPainter {
  ...
  void _egg(Canvas canvas) {
    ...
    final shell = Path()
      ...
      ..close();
    canvas.drawPath(shell, _linear(const Rect.fromLTWH(22, 10, 58, 74), const <Color>[Color(0xFFFFF9E9), Color(0xFFE8D6AF)]));
    canvas.drawPath(shell, _stroke(const Color(0xFFC8B48E), 1.6));
    canvas.drawOval(const Rect.fromLTWH(52, 48, 36, 27), _fill(const Color(0xFFF8F7F0)));
    canvas.drawOval(const Rect.fromLTWH(63, 54, 16, 16), _radial(const Rect.fromLTWH(63, 54, 16, 16), const Color(0xFFFFD75E), const Color(0xFFE79A14)));
  }

  void _wholeGrainBread(Canvas canvas) {
    _shadow(canvas, const Rect.fromLTWH(15, 73, 70, 11));
    final r = RRect.fromRectAndRadius(const Rect.fromLTWH(17, 26, 66, 48), const Radius.circular(18));
  ...
  void _chicken(Canvas canvas, {required bool grilled}) {
    ...
    final body = RRect.fromRectAndRadius(const Rect.fromLTWH(20, 25, 48, 48), const Radius.circular(19));
    canvas.drawRRect(body, _linear(const Rect.fromLTWH(20, 25, 48, 48), const <Color>[Color(0xFFF1AF68), Color(0xFFC66F37)]));
    canvas.drawRRect(body, _stroke(const Color(0xFF9B4F29), 1.6));
    canvas.drawRRect(RRect.fromRectAndRadius(const Rect.fromLTWH(62, 50, 22, 12), const Radius.circular(5)), _fill(const Color(0xFFF2E2C6)));
    canvas.drawOval(const Rect.fromLTWH(79, 47, 10, 10), _fill(const Color(0xFFF5E7CD)));
    canvas.drawOval(const Rect.fromLTWH(81, 57, 10, 10), _fill(const Color(0xFFF5E7CD)));
    if (grilled) {
      for (final x in <double>[29, 39, 49, 59]) {
        canvas.drawLine(Offset(x, 33), Offset(x + 12, 62), _stroke(const Color(0xFF6B3923), 2.4));
      }
    }
  }
  ...
  void _beef(Canvas canvas) {
    ...
    final steak = Path()
      ..moveTo(19, 46)..lineTo(30, 28)..lineTo(56, 22)..lineTo(78, 34)..lineTo(84, 52)..lineTo(69, 69)..lineTo(43, 77)..lineTo(21, 66)..close();
    canvas.drawPath(steak, _linear(const Rect.fromLTWH(18, 22, 67, 55), const <Color>[Color(0xFFCB7768), Color(0xFF7D2E28)]));
    canvas.drawPath(steak, _stroke(const Color(0xFF68241F), 1.8));
    for (final x in <double>[38, 50, 62]) {
      canvas.drawLine(Offset(x, 33), Offset(x + 10, 61), _stroke(const Color(0xFF69372F), 2.2));
    }
  }
  ...
  void _salmon(Canvas canvas) {
    ...
    final p = Path()..moveTo(14, 55)..lineTo(28, 32)..lineTo(63, 29)..lineTo(85, 45)..lineTo(70, 66)..lineTo(31, 72)..close();
    canvas.drawPath(p, _linear(const Rect.fromLTWH(14, 29, 71, 43), const <Color>[Color(0xFFFF9D78), Color(0xFFD95D42)]));
    for (final x in <double>[34, 44, 54, 64, 74]) {
      canvas.drawLine(Offset(x, 34), Offset(x - 7, 63), _stroke(const Color(0xFFFFD4C1), 1.7));
    }
  }

  void _milk(Canvas canvas) {
    _shadow(canvas, const Rect.fromLTWH(28, 74, 44, 9));
    final glass = RRect.fromRectAndRadius(const Rect.fromLTWH(32, 18, 36, 60), const Radius.circular(7));
    canvas.drawRRect(glass, _fill(const Color(0xFFEAF3F0)));
    canvas.drawRRect(glass, _stroke(const Color(0xFFB5C7C1), 1.4));
    canvas.drawRect(const Rect.fromLTWH(35, 31, 30, 42), _linear(const Rect.fromLTWH(35, 31, 30, 42), const <Color>[Color(0xFFFFFEF5), Color(0xFFEFEFDD)]));
    canvas.drawOval(const Rect.fromLTWH(35, 27, 30, 8), _fill(const Color(0xFFFBFBF1)));
  }

  void _yogurt(Canvas canvas) {
    _shadow(canvas, const Rect.fromLTWH(20, 72, 60, 10));
    canvas.drawOval(const Rect.fromLTWH(19, 43, 62, 31), _fill(const Color(0xFFE7ECE9)));
    canvas.drawOval(const Rect.fromLTWH(23, 36, 54, 22), _fill(const Color(0xFFFAFAF4)));
  }
  ...
  void _banana(Canvas canvas) {
    ...
    final outer = Paint()..style = PaintingStyle.stroke..strokeCap = StrokeCap.round..strokeWidth = 20..color = const Color(0xFFF0C02F);
    final inner = Paint()..style = PaintingStyle.stroke..strokeCap = StrokeCap.round..strokeWidth = 12..color = const Color(0xFFFFE15C);
    const r = Rect.fromLTWH(18, 15, 64, 60);
    canvas.drawArc(r, .15, 2.25, false, outer);
    canvas.drawArc(r, .18, 2.18, false, inner);
  }
  ...
  void _bowl(Canvas canvas, {required Color soup, bool lentils = false, bool chickpeas = false, bool harira = false}) {
    _shadow(canvas, const Rect.fromLTWH(18, 73, 64, 10));
    canvas.drawOval(const Rect.fromLTWH(20, 42, 60, 37), _fill(const Color(0xFFE7ECEA)));
    canvas.drawOval(const Rect.fromLTWH(24, 34, 52, 24), _fill(soup));
    canvas.drawArc(const Rect.fromLTWH(23, 33, 54, 26), 0, math.pi, false, _stroke(_teal, 1.7));
    final count = lentils ? 15 : chickpeas ? 10 : 8;
    for (var i = 0; i < count; i++) {
      final a = i * 2.3;
      final r = 5 + (i % 5) * 2.5;
      final color = lentils ? const Color(0xFFA66D3D) : chickpeas ? const Color(0xFFE4BE78) : i.isEven ? const Color(0xFFDCB16E) : const Color(0xFF4D925A);
      canvas.drawOval(Rect.fromCenter(center: Offset(50 + math.cos(a) * r, 47 + math.sin(a) * r * .36), width: 3.2, height: 2.4), _fill(color));
    }
    if (harira) {
      canvas.drawArc(const Rect.fromLTWH(34, 40, 32, 13), .4, 1.8, false, _stroke(Colors.white.withValues(alpha: .22), 1));
    }
  }

  void _oliveOil(Canvas canvas) {
    _shadow(canvas, const Rect.fromLTWH(29, 73, 42, 10));
    final bottle = RRect.fromRectAndRadius(const Rect.fromLTWH(34, 23, 32, 54), const Radius.circular(7));
    canvas.drawRRect(bottle, _fill(const Color(0xFFD9E3C9)));
    canvas.drawRRect(bottle, _stroke(const Color(0xFF6D7A4B), 1.5));
    canvas.drawRect(const Rect.fromLTWH(38, 47, 24, 26), _linear(const Rect.fromLTWH(38, 47, 24, 26), const <Color>[Color(0xFFD9B34A), Color(0xFF9D7B22)]));
    canvas.drawRect(const Rect.fromLTWH(40, 14, 20, 12), _fill(const Color(0xFF355A3C)));
    canvas.drawRRect(RRect.fromRectAndRadius(const Rect.fromLTWH(42, 9, 16, 7), const Radius.circular(2)), _fill(const Color(0xFF28472F)));
  }
  ...
  void _moroccanBread(Canvas canvas) {
    ...
    const b = Rect.fromLTWH(19, 22, 62, 57);
    canvas.drawOval(b, _linear(b, const <Color>[Color(0xFFE0B06E), Color(0xFFA7632D)]));
    canvas.drawOval(b, _stroke(const Color(0xFF7C461F), 1.5));
    for (final x in <double>[37,50,63]) {
      canvas.drawArc(Rect.fromLTWH(x-8,32,16,30), .9, 1.4, false, _stroke(const Color(0xFFF1D29C), 2.5));
    }
  }

  void _msemen(Canvas canvas) {
    _shadow(canvas, const Rect.fromLTWH(17, 74, 66, 10));
    final r = RRect.fromRectAndRadius(const Rect.fromLTWH(20,25,60,53), const Radius.circular(7));
    canvas.drawRRect(r, _linear(const Rect.fromLTWH(20,25,60,53), const <Color>[Color(0xFFF2C06F), Color(0xFFC8863D)]));
    canvas.drawRRect(r, _stroke(const Color(0xFF9F6028), 1.5));
    for (final y in <double>[38,48,58,68]) {
      canvas.drawLine(Offset(28,y), Offset(72,y-3), _stroke(const Color(0xFFAD6B30), 1));
    }
  }

  void _baghrir(Canvas canvas) {
    _shadow(canvas, const Rect.fromLTWH(17, 74, 66, 10));
    const b = Rect.fromLTWH(18,25,64,53);
  ...
  void _couscous(Canvas canvas) {
    ...
    canvas.drawOval(const Rect.fromLTWH(14,37,72,46), _fill(const Color(0xFFF6F8F6)));
    canvas.drawOval(const Rect.fromLTWH(22,34,56,35), _fill(const Color(0xFFE1C67F)));
    ...
    for (final e in vegetables) {
      canvas.drawRRect(RRect.fromRectAndRadius(e.$1, const Radius.circular(2)), _fill(e.$2));
    }
    canvas.drawArc(const Rect.fromLTWH(14,37,72,46), 0, math.pi, false, _stroke(_teal, 1.4));
  }
