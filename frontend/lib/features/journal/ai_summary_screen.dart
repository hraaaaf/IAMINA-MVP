...
class _AISummaryScreenState extends State<AISummaryScreen> {
  ...
  Future<void> _fetchData() async {
    ...
    setState(() {
      ...
      _kpis    = kpis;
      _isLoading = false;
      if (summary == null) _errorMessage = 'Impossible de récupérer les analyses.';
    });
  }
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          if (_showTweaks)
            Positioned.fill(child: TweaksPanel(onClose: () => setState(() => _showTweaks = false))),
        ],
      ),
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text('IAmina analyse vos données…', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: AminaTheme.textPrimary(context))),
          const SizedBox(height: 6),
          Text('Cela prend quelques secondes.', style: TextStyle(color: AminaTheme.textSecondary(context), fontSize: 13)),
        ],
      ),
    );
  }
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: const Icon(Icons.error_outline, color: AminaTheme.dangerFg, size: 26),
            ),
            const SizedBox(height: 16),
            Text(_errorMessage!, textAlign: TextAlign.center, style: TextStyle(fontSize: 14, color: AminaTheme.textSecondary(context))),
  ...
  // ── Mobile layout ──────────────────────────────────────────────────────────
  Widget _buildNarrowLayout(SummaryResponse summary, List<InsightCard> cards, KpisResponse? kpis) {
  ...
  List<Widget> _buildAnalyticsSection(SummaryResponse summary, KpisResponse? kpis) {
    final agpData  = summary.agpProfile.isNotEmpty ? summary.agpProfile : summary.dailyAverages;
    ...
    return [
      ...
      if (agpData.isNotEmpty) ...[
        _AgpCard(agpData: agpData, isHourly: useHourly, periodDays: _periodDays, kpis: kpis),
        const SizedBox(height: 16),
      ],
    ];
  }
      ...
      if (cards.isEmpty)
        ClinicalCard(
          child: Padding(
            ...
            child: Center(child: Text('Aucune découverte pour le moment.', style: TextStyle(color: AminaTheme.textSecondary(context)))),
          ),
        )
      else
          ...
          Row(
            children: [
              Expanded(
                child: RichText(
                  text: TextSpan(
                    style: TextStyle(fontFamily: 'Inter', fontSize: 13, color: AminaTheme.textSecondary(context)),
                    children: [
              ...
              Row(
                children: [
                  _PeriodTab(label: '7j', days: 7, selected: periodDays == 7, onTap: onPeriodChange),
                  const SizedBox(width: 4),
                  _PeriodTab(label: '21j', days: 21, selected: periodDays == 21, onTap: onPeriodChange),
                  const SizedBox(width: 4),
                  _PeriodTab(label: '90j', days: 90, selected: periodDays == 90, onTap: onPeriodChange),
                ],
              ),
...
class _PeriodTab extends StatelessWidget {
  ...
  const _PeriodTab({required this.label, required this.days, required this.selected, required this.onTap});
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: selected ? Colors.black : AminaTheme.divider(context)),
        ),
        child: Text(label, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: selected ? Colors.white : AminaTheme.textSecondary(context))),
      ),
    );
  }
}
...
class _GreetingHeader extends StatelessWidget {
  ...
  String _firstName() {
    ...
    return display.split(RegExp(r'[\s@.]')).firstWhere(
      (p) => p.isNotEmpty,
      orElse: () => '',
    );
  }
...
class _HeroButton extends StatelessWidget {
  ...
  const _HeroButton({required this.label, required this.onTap, required this.isPrimary});
...
class _KpiRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final tirColor = tir >= 70 ? AminaTheme.teal500 : tir >= 50 ? AminaTheme.ambre500 : AminaTheme.dangerFg;
    final gmiColor = gmi < 7  ? AminaTheme.teal500  : gmi < 8  ? AminaTheme.ambre500 : AminaTheme.dangerFg;
    final cvColor  = cv < 36  ? AminaTheme.teal500  : cv < 40  ? AminaTheme.ambre500 : AminaTheme.dangerFg;

    return Row(children: [
      Expanded(child: _KpiCard(label: 'TIME IN RANGE', abbr: '${tir.toStringAsFixed(0)}%', value: '${tir.toStringAsFixed(0)}%', color: tirColor, progress: tir / 100, target: '70-180 mg/dL', trend: tir >= 70 ? 1 : -1)),
      const SizedBox(width: 12),
      Expanded(child: _KpiCard(
        ...
        target: kpis.gmiBasis.isNotEmpty ? kpis.gmiBasis : '${kpis.daysWithData}j',
        trend: gmi < 7 ? 1 : -1,
        confidenceBadge: _buildGmiBadge(context, kpis.gmiConfidence),
      )),
      const SizedBox(width: 12),
      Expanded(child: _KpiCard(label: 'VARIABILITÉ (CV)', abbr: '${cv.toStringAsFixed(0)}%', value: '${cv.toStringAsFixed(0)}%', color: cvColor, progress: (cv / 50).clamp(0.0, 1.0), target: '<36% ADA', trend: cv < 36 ? 1 : -1)),
    ]);
  }
}
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Text(label, style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: AminaTheme.textSecondary(context), letterSpacing: 0.5)),
          Text(target, style: TextStyle(fontSize: 10, color: AminaTheme.textSecondary(context))),
        ]),
        const SizedBox(height: 12),
        Row(crossAxisAlignment: CrossAxisAlignment.end, children: [
          Text(value, style: TextStyle(fontSize: 42, fontWeight: FontWeight.w800, color: AminaTheme.textPrimary(context), letterSpacing: -1.5, height: 1.0)),
...
class _AgpCard extends StatelessWidget {
  ...
  const _AgpCard({required this.agpData, required this.isHourly, required this.periodDays, this.kpis});
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Padding(
          ...
          child: Row(children: [
            ...
            const SizedBox(width: 8),
            Expanded(child: Text('PROFIL GLYCÉMIQUE AMBULATOIRE', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: AminaTheme.textSecondary(context), letterSpacing: 0.6))),
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
              decoration: BoxDecoration(color: AminaTheme.teal50, borderRadius: BorderRadius.circular(99)),
              child: Text('${periodDays}j', style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: AminaTheme.teal600)),
            ),
          ]),
        ),
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ClipRRect(
                ...
                child: SizedBox(height: 8, child: Row(children: [
                  if (tbr > 0)  Flexible(flex: tbr.round(),  child: Container(color: AminaTheme.dangerFg)),
                  if (tir > 0)  Flexible(flex: tir.round(),  child: Container(color: AminaTheme.teal500)),
                  if (tar > 0)  Flexible(flex: tar.round(),  child: Container(color: AminaTheme.ambre500)),
                  if (vtar > 0) Flexible(flex: vtar.round(), child: Container(color: AminaTheme.dangerFg.withValues(alpha: 0.6))),
                ])),
              ),
              const SizedBox(height: 6),
              Row(children: [
                _TirLegend(color: AminaTheme.teal500,  label: 'En cible', value: '${tir.toStringAsFixed(0)}%'),
                const SizedBox(width: 10),
                _TirLegend(color: AminaTheme.ambre500, label: 'Élevé',    value: '${tar.toStringAsFixed(0)}%'),
                const SizedBox(width: 10),
                _TirLegend(color: AminaTheme.dangerFg, label: 'Bas',      value: '${tbr.toStringAsFixed(0)}%'),
                const Spacer(),
                Text('Objectif ADA > 70%', style: TextStyle(fontSize: 9, color: tir >= 70 ? AminaTheme.teal500 : AminaTheme.textSecondary(context))),
                if (tir >= 70) ...[const SizedBox(width: 4), const Icon(Icons.check_circle, size: 11, color: AminaTheme.teal500)],
              ]),
              const SizedBox(height: 10),
            ]),
          ),
        if (!hasData)
          ...
          Builder(builder: (ctx) {
            ...
            const minY = 40.0; const maxY = 280.0;
            final pts = agpData
                .map((e) => e as Map<String, dynamic>)
                .where((m) => (m['p50'] ?? m['avg'] ?? 0) > 0)
                .map((m) {
                  num v(String k) => m[k] as num? ?? 0;
                  final p50 = (v('p50') > 0 ? v('p50') : v('avg')).toDouble();
                  return AgpPoint(
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(children: [
                        ...
                        if (isHourly)
                          SizedBox(height: 16, child: AgpXAxis(isDark: isDark)),
                      ]),
                    ),
                  ],
                ),
        ...
        Padding(
          ...
          child: Row(children: [
            ...
            const SizedBox(width: 10),
            _LegendDot(color: AminaTheme.teal400.withValues(alpha: 0.55),           label: '25–75%'),
            const SizedBox(width: 10),
            _LegendDot(color: AminaTheme.teal400.withValues(alpha: 0.25),           label: '5–95%'),
            const Spacer(),
            Text('Cible 70–180', style: TextStyle(fontSize: 9, color: AminaTheme.textSecondary(context))),
          ]),
        ),
      ]),
    );
...
class _LegendDot extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(children: [
    ...
    const SizedBox(width: 4),
    Text(label, style: TextStyle(fontSize: 9, color: AminaTheme.textSecondary(context))),
  ]);
}
...
class _TirLegend extends StatelessWidget {
  ...
  const _TirLegend({required this.color, required this.label, required this.value});
  @override
...
class _ActionPlan extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final actionCards = cards.where((c) => c.action.isNotEmpty).take(7).toList();
    ...
    final List<_PlanItem> plans = actionCards.isEmpty
        ...
        : actionCards.asMap().entries.map((e) {
            ...
            return _PlanItem(
              day:   'J+$dayOffset',
              title: card.action,
              sub:   card.body.length > 60 ? '${card.body.substring(0, 60)}…' : card.body,
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('PLAN D\'ACTION', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: AminaTheme.textSecondary(context), letterSpacing: 0.12)),
            const SizedBox(height: 4),
            Text('$count ajustement${count > 1 ? 's' : ''} · 7 prochains jours', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AminaTheme.textPrimary(context))),
          ])),
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 7),
            decoration: BoxDecoration(border: Border.all(color: AminaTheme.divider(context)), borderRadius: BorderRadius.circular(99)),
            child: Text('Partager au médecin', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: AminaTheme.textSecondary(context))),
          ),
        ]),
          ...
          child: Container(
            ...
            child: Row(children: [
              ...
              Container(
                ...
                child: Text(p.day, style: TextStyle(fontSize: 10.5, fontWeight: FontWeight.w700, color: p.dot, letterSpacing: 0.06)),
              ),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(p.title, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: AminaTheme.textPrimary(context))),
                const SizedBox(height: 2),
                Text(p.sub, style: TextStyle(fontSize: 11, color: AminaTheme.textSecondary(context), height: 1.3)),
              ])),
...
class _PlanItem {
  ...
  final Color bg;
  final Color dot;
  const _PlanItem({required this.day, required this.title, required this.sub, required this.bg, required this.dot});
}
...
class _SectionHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(children: [
      ...
      const SizedBox(width: 10),
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(title, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: AminaTheme.textSecondary(context), letterSpacing: 0.8)),
        Text(subtitle, style: TextStyle(fontSize: 12, color: AminaTheme.textSecondary(context))),
      ])),
    ]);
  }
}
...
class _InsightCardWidgetState extends State<_InsightCardWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    final (barColor, bgColor, severityLabel, severityIcon) = switch (card.severity) {
      InsightSeverity.good   => (AminaTheme.goodFg,  AminaTheme.goodBg,  'Positif',        Icons.check_circle_outline),
      InsightSeverity.warn   => (AminaTheme.warnFg,  AminaTheme.warnBg,  'À surveiller',   Icons.radio_button_unchecked),
      InsightSeverity.danger => (AminaTheme.dangerFg, AminaTheme.dangerBg, 'Priorité haute', Icons.warning_amber_rounded),
    };
        ...
        InkWell(
          ...
          child: Padding(
            ...
            child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Expanded(
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text(card.title, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: AminaTheme.textPrimary(context), height: 1.3)),
                  const SizedBox(height: 5),
                  Row(children: [
                    Container(
                      padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 2),
                      decoration: BoxDecoration(color: bgColor, borderRadius: BorderRadius.circular(99)),
                      child: Text(severityLabel, style: TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: barColor)),
                    ),
                    const SizedBox(width: 8),
                    _SignalBars(color: barColor),
                    const SizedBox(width: 6),
                    Text('conf. ${_confidenceForSeverity(card.severity)}%', style: TextStyle(fontSize: 10, color: AminaTheme.textSecondary(context))),
                  ]),
                ]),
              ),
              Icon(_expanded ? Icons.keyboard_arrow_up : Icons.keyboard_arrow_down, size: 20, color: AminaTheme.textSecondary(context)),
            ]),
          ),
        ),
        AnimatedCrossFade(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if (card.action.isNotEmpty) ...[
                ...
                Container(
                  ...
                  child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    const Icon(Icons.check_circle_outline, size: 14, color: AminaTheme.teal600),
                    const SizedBox(width: 8),
              ...
              Row(children: [
                _ActionBtn(
                  ...
                  onTap: () => setState(() { _accepted = !_accepted; _ignored = false; }),
                ),
                const SizedBox(width: 8),
                _ActionBtn(
                  label: 'Ignorer',
                  ...
                  onTap: () => setState(() { _ignored = !_ignored; _accepted = false; }),
                ),
                const Spacer(),
                GestureDetector(
                  onTap: () => widget.onAskWhy(),
                  child: const Text('Pourquoi ?', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: AminaTheme.teal600)),
                ),
              ]),
            ]),
          ),
...
class _ActionBtn extends StatelessWidget {
  ...
  final VoidCallback onTap;
  const _ActionBtn({required this.label, required this.icon, required this.active, required this.activeColor, required this.onTap});
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: active ? activeColor : AminaTheme.divider(context)),
          borderRadius: BorderRadius.circular(99),
        ),
        child: Row(children: [
          Icon(icon, size: 12, color: active ? Colors.white : AminaTheme.textSecondary(context)),
          const SizedBox(width: 4),
          Text(label, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: active ? Colors.white : AminaTheme.textSecondary(context))),
        ]),
      ),
    );
  }
    ...
    return Container(
      ...
      child: Row(children: [
        const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Discuter avec IAmina', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: Colors.white)),
          SizedBox(height: 4),
          Text('Posez vos questions ou demandez des conseils personnalisés.', style: TextStyle(fontSize: 12, color: Colors.white70, height: 1.4)),
        ])),
        const SizedBox(width: 12),
        FilledButton(
          onPressed: onTap,
          ...
          child: const Text('Démarrer', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700)),
        ),
      ]),
    );
  }
          ...
          child: Stack(alignment: Alignment.center, children: [
            Container(
              ...
              decoration: BoxDecoration(
                ...
                color: AminaTheme.teal500.withValues(alpha: (1.0 - ripple) * 0.28),
              ),
            ),
            Container(
              width: 56, height: 56,
...
class _SignalBars extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(
    ...
    children: [for (int i = 0; i < 3; i++) ...[
      Container(width: 4, height: 6.0 + i * 4, decoration: BoxDecoration(color: color.withValues(alpha: 0.9), borderRadius: BorderRadius.circular(2))),
