...
void main(){
 ...
 test('B24 appends three Morocco concepts after certified B23 baseline',(){expect(mealFoodCatalog.length,374);expect(mealFoodCatalog.take(371).length,371);expect(mealFoodCatalog.skip(371).map((e)=>e.id).toList(),ids);expect(mealFoodCatalog.map((e)=>e.id).toSet().length,374);});
