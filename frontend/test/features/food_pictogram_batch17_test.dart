...
void main(){
  ...
  test('B17 remains the exact four-concept prefix after certified B16',(){
    expect(baseline.mealFoodCatalog,hasLength(322)); expect(gulfCoreFoodCatalog,hasLength(24)); expect(moroccoRegionalB16FoodCatalog,hasLength(3)); expect(moroccoDepthB17FoodCatalog,hasLength(4)); expect(mealFoodCatalog.length,greaterThanOrEqualTo(353));
    ...
    expect(mealFoodCatalog.skip(349).take(4).map((e)=>e.id).toSet(),expected);
  });
  test('B17 labels, search, categories and native pictograms remain complete',(){
    expect(mealFoodCatalog.take(353).map((e)=>e.id).toSet(),hasLength(353)); expect(codeFoodPictogramBatch17Ids,expected);
