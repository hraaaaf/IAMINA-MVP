...
void main(){
  const expected=<String>{'tanjia_marrakchia','chebakia','lben_moroccan','raib_moroccan'};
  ...
  test('B17 labels, search, categories and native pictograms are complete',(){
    ...
    expect(mealFoodById('tanjia_marrakchia')!.category,MealFoodCategory.moroccanDish); expect(mealFoodById('chebakia')!.category,MealFoodCategory.sweetDessert); expect(mealFoodById('lben_moroccan')!.category,MealFoodCategory.dairy); expect(mealFoodById('raib_moroccan')!.category,MealFoodCategory.dairy);
  });
}
