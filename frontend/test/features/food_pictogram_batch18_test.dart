...
void main(){
  ...
  test('B18 remains the exact three-concept prefix after certified B17',(){
    expect(baseline.mealFoodCatalog,hasLength(322));expect(gulfCoreFoodCatalog,hasLength(24));expect(moroccoRegionalB16FoodCatalog,hasLength(3));expect(moroccoDepthB17FoodCatalog,hasLength(4));expect(moroccoAmazighB18FoodCatalog,hasLength(3));expect(mealFoodCatalog.length,greaterThanOrEqualTo(356));
    ...
    expect(mealFoodCatalog.skip(353).take(3).map((e)=>e.id).toSet(),expected);
  });
  test('B18 labels, search, categories and native pictograms remain complete',(){
    expect(mealFoodCatalog.map((e)=>e.id).toSet(),hasLength(mealFoodCatalog.length));expect(codeFoodPictogramBatch18Ids,expected);
