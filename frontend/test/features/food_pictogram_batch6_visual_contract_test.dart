...
void main() {
  test('native food pictogram batches are fixed, disjoint and total 144', () {
    ...
    for (final batch in batches) expect(batch.length, 24);
    for (var i = 0; i < batches.length; i++) {
      for (var j = i + 1; j < batches.length; j++) {
        expect(batches[i].intersection(batches[j]), isEmpty);
      }
    }
    expect(<String>{for (final batch in batches) ...batch}.length, 144);
  });
}
