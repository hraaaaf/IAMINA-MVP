...
void main() {
  ...
  test('catalog v3 is broad, unique and Morocco + GCC aware', () {
    expect(mealFoodCatalogVersion, '3.2.0-morocco-regional-b16');
  ...
  test('every food has a non-generic visual cue and keeps a text label', () {
    for (final item in mealFoodCatalog) {
      ...
      for (final locale in const <Locale>[Locale('fr'), Locale('en'), Locale('ar')]) {
  ...
  testWidgets('photo proposal never becomes meal data before explicit confirmation', (tester) async {
    var selected = <String>[];
    Future<MealAnalysisResult?> recognition() async => const MealAnalysisResult(foods: <String>['pain complet', 'œuf'], confidence: 'high', fallback: false);
    await tester.pumpWidget(harness(selected: selected, onChanged: (value) => selected = value, photoRecognition: recognition, favoritesRepository: _MemoryFavoritesRepository()));
    ...
    await tester.tap(find.byKey(const Key('meal-photo-candidate-whole_grain_bread')));
    await tester.pump();
    expect(selected, isEmpty);
  ...
  testWidgets('Arabic search uses Arabic food labels and RTL', (tester) async {
    var selected = <String>[];
    await tester.pumpWidget(harness(locale: const Locale('ar'), selected: selected, onChanged: (value) => selected = value, favoritesRepository: _MemoryFavoritesRepository()));
    await tester.pumpAndSettle();
    ...
    expect(find.descendant(of: result, matching: find.text('بيض')), findsWidgets);
    expect(find.text('Œuf'), findsNothing);
  });

  testWidgets('first use hides empty history and keeps capture actions immediately available', (tester) async {
    await tester.pumpWidget(harness(selected: const <String>[], onChanged: (_) {}, favoritesRepository: _MemoryFavoritesRepository()));
    await tester.pumpAndSettle();
  ...
  testWidgets('Gulf category browses regional dishes without typing', (tester) async {
    await tester.pumpWidget(harness(selected: const <String>[], onChanged: (_) {}, favoritesRepository: _MemoryFavoritesRepository()));
    await tester.pumpAndSettle();
  ...
  testWidgets('recent foods come from structured history and habitual duplicates are suppressed', (tester) async {
    for (var i = 0; i < 3; i++) {
      await db.into(db.logEntries).insert(LogEntriesCompanion.insert(createdAt: DateTime(2026, 8, 9, 8 + i), bloodSugar: 120.0 + i, clientUuid: '88888888-8888-8888-8888-88888888888$i', mealItemsJson: const Value('["egg","moroccan_bread"]'), loggedAt: Value(DateTime(2026, 8, 9, 8 + i))));
    }
    await tester.pumpWidget(harness(selected: const <String>[], onChanged: (_) {}, favoritesRepository: _MemoryFavoritesRepository()));
    await tester.pumpAndSettle();
  ...
  testWidgets('search result selects food and collapses active results', (tester) async {
    var selected = <String>[];
    final favorites = _MemoryFavoritesRepository();
    void onChanged(List<String> value) => selected = value;
    await tester.pumpWidget(harness(selected: selected, onChanged: onChanged, favoritesRepository: favorites));
    await tester.pumpAndSettle();
    ...
    final field = tester.widget<TextField>(find.byKey(const Key('meal-food-search')));
    expect(field.controller?.text, isEmpty);
  });

  testWidgets('favorite can be added from search and surfaces in quick access', (tester) async {
    final favorites = _MemoryFavoritesRepository();
    await tester.pumpWidget(harness(selected: const <String>[], onChanged: (_) {}, favoritesRepository: favorites));
    await tester.pumpAndSettle();
    await tester.enterText(find.byKey(const Key('meal-food-search')), 'oeuf');
    await tester.pump();
  ...
  testWidgets('favorite quick access can remove a stored favorite', (tester) async {
    final favorites = _MemoryFavoritesRepository(<String>{'whole_grain_bread'});
    await tester.pumpWidget(harness(selected: const <String>[], onChanged: (_) {}, favoritesRepository: favorites));
    await tester.pumpAndSettle();
    expect(find.byKey(const Key('meal-favorites-section')), findsOneWidget);
    expect(find.byKey(const Key('meal-favorite-row-whole_grain_bread')), findsOneWidget);
    await tester.tap(find.byKey(const Key('meal-favorite-whole_grain_bread')));
    await tester.pumpAndSettle();
