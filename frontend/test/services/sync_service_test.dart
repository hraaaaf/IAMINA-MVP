...
void main() {
  ...
  test(
    'syncPendingLogs marks log as synced when UUID returned by server',
    () async {
      when(() => mockDb.getPendingLogs()).thenAnswer((_) async => [testLog]);
      when(
        () => mockApi.batchSyncLogs(any()),
  ...
  test(
    'syncPendingLogs reports failure when server does not confirm UUID',
    () async {
      when(() => mockDb.getPendingLogs()).thenAnswer((_) async => [testLog]);
      when(
        () => mockApi.batchSyncLogs(any()),
      ...
      verify(() => mockApi.batchSyncLogs(any())).called(1);
      verify(
        () => mockDb.reportSyncFailure(testLog.id, testLog.syncAttempts),
      ).called(1);
  ...
  test(
    'syncPendingLogs handles multiple logs — syncs matching, fails unmatched',
    () async {
      ...
      when(
        () => mockDb.getPendingLogs(),
      ).thenAnswer((_) async => [testLog, log2]);
