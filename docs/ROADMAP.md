...
| P0-MENA-2 — locale + safety contract | 63% | 🟡 Linguistic evidence retained; clinical-human release gate open | English baseline 16/16 certified; P5-1/#515 Morocco linguistic gate is CLOSED with exact-main v8 evidence; #318 remains `BLOCKED_EXTERNAL_HUMAN` for the independent qualified clinical-human release review |
...
7. **P5-6 — Real-patient release gate — ACTIVE / BLOCKED_EXTERNAL / READY_FOR_CANDIDATE_FREEZE.** P5-1 prerequisite is closed. After this closeout merges and exact-main CI is green, freeze the resulting `main` SHA in #318 and #320. The current-tree fail-closed commands `audit_pilot_consent_governance --require-approved`, `audit_pilot_data_residency --require-approved` and `audit_safety_corpus_review --require-approved` remain the release gates. No approved restricted manifests or external release decision are claimed. Release posture remains `NOT_RELEASE_AUTHORIZED`. Canonical gate: `docs/P5_6_REAL_PATIENT_RELEASE_GATE.md`.
...
`P5-6 candidate freeze → #318 clinical-human approval + #320 CNDP/processor/residency approval → 3 exact-SHA approved audits → explicit human release decision → P5-7 observed pilot evidence → P5-8 go/no-go`
...
**Pilot Readiness progress: 4/9 = 44.4%.** Closed whole lots are P5-0, P5-1, P5-2 and P5-5. P5-4A is a closed sub-lot, but P5-4 remains split/open because P5-4B is deferred; therefore the P5 numerator does not include P5-4. P5-6 remains blocked on external/human release evidence. This metric is separate from the retained 32/38 MENA numerator.
