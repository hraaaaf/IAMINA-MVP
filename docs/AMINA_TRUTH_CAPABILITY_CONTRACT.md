...
> **Status:** chassis contract introduced by P0.2, extended to structured insight formatting by P0.3 and enforced at the legacy-memory boundary by P0.4.  
...
Neither a heuristic nor a generative inference may become a patient fact, clinical rule or durable clinical truth merely because it was plausible, repeated or persisted.
...
| `HEURISTIC_INFERENCE` | non-authoritative inference from a heuristic without an approved clinical-derivation contract | no | no |
...
A companion-memory snapshot is **not** a patient-fact store. If non-clinical runtime state, a recomputable derivative or a compatibility-only heuristic is cached there, it must carry an explicit truth kind and stable source and must never gain additional clinical authority merely because it survived a restart.
...
P0.4 migrates the legacy companion snapshot boundary without changing the database schema. `memory` and `deep` continue to use the condition-agnostic `SnapshotStore` and existing JSON fields, while new writes use the versioned `iamina.companion-memory` v2 envelope defined in `backend/companion/memory_truth.py`.
...
Historical `food_sensitivities` and `peak_hours` are compatibility-only **heuristic inferences**, not approved deterministic clinical derivations. They remain structurally readable but are labelled `HEURISTIC_INFERENCE`, cannot enter deterministic clinical logic or patient-fact persistence, and do not gain authority from the snapshot envelope. The IAmina orchestrator no longer learns the legacy food-response heuristic and `compute_state()` no longer turns it into a meal-related intention.

PR #114 briefly emitted the exact same legacy fields with `DETERMINISTIC_DERIVATION` provenance. P0.4-R1 accepts those exact markers only as a backward-compatible read alias and rewrites the corrected `HEURISTIC_INFERENCE` marker on the next encode/save; arbitrary provenance mismatches still fail closed.

Detailed implementation and acceptance evidence live in `docs/P0_4_LEGACY_MEMORY_TRUTH_MIGRATION.md` and the focused P0.4-R1 regression tests.
...
- legacy heuristic fields are never classified as approved deterministic clinical derivations;
- the exact pre-remediation PR #114 v2 markers remain readable only as migration aliases and re-encode to `HEURISTIC_INFERENCE`.
