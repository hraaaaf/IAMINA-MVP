...
- **Current compromise:** other large Flutter widgets/services remain from rapid iteration; TD-012 therefore stays open after the Dashboard trend, Add Log, AISummary, IAmina chat, Profile, DocumentImport, Companion and Reports splits. Reports is merged by PR #698 as `6920c20b67af466ac69a1be292a9afcd3b17ca91`: `reports_screen.dart` is reduced from ~29.0k to ~3.8k characters with presentation widgets moved to `reports_screen_presentation.dart`; exact-head CI #4599, browser #970, UI geometry #855, Offline UI #97 and P5-5 #354 passed. Visual comparison was identical at 390×844; tablet and desktop differed only in the time-dependent timestamp text (158 pixels each), with no layout/geometry change; retained visual score 9.9/10. `ai_summary_screen_presentation.dart` remains large (~55.6k) but is presentation-only after PR #689. Generated localization/database files and static catalogs are excluded from this maintainability hotspot list.
