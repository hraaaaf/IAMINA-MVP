...
- **Current compromise:** other large Flutter widgets/services remain from rapid iteration; TD-012 therefore stays open after the Dashboard trend, Add Log, AISummary, IAmina chat, Profile and DocumentImport splits. DocumentImport is merged by PR #692 as `1e88bf6ac9ab1206bf26401bd777092f46f75ef4`: `document_import_screen.dart` is reduced from ~31.2k to ~14.7k characters with UI helpers moved to `document_import_screen_presentation.dart`; exact-head CI #4576, browser #954, UI geometry #840 and P5-5 #338 passed, and BEFORE/AFTER at 390×844, 768×1024 and 1280×900 were pixel-identical (0 differing pixels). The next verified orchestration/UI hotspot is `companion_premium_screen.dart` (~30.0k), followed by `reports_screen.dart` (~29.6k). `ai_summary_screen_presentation.dart` remains large (~55.6k) but is presentation-only after PR #689. Generated localization/database files and static catalogs are excluded from this maintainability hotspot list.
