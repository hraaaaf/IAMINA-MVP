...
P2-DOCTOR-1 adds a read-only diabetes-owned assembler for CURRENT_SNAPSHOT only. It reads synchronized non-demo Journal glucose rows and already-persisted governed Clinical Twin state, computes only the governed descriptive exact-window recorded-glucose average database-side, and does not refresh or mutate source state. Historical requests fail closed on Clinical Twin rows refreshed after the requested window end. Because no authoritative clinician-review checkpoint provider exists, every checkpoint input is rejected and since-review semantics remain unreachable. P2-DOCTOR-2 must establish that authority source before the assembler can be extended; API/narrator/UX integration remains later work.
