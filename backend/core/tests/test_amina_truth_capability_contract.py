...
def test_observed_fact_and_user_claim_keep_distinct_authority():
    ...
    assert observed.may_drive_clinical_decision is True
    assert claim.may_persist_as_patient_fact is True
    assert claim.may_drive_clinical_decision is False

    claim.assert_patient_fact_persistence_allowed()
    with pytest.raises(ValueError, match="not clinical decision authority"):
        claim.assert_clinical_authority()


def test_deterministic_derivation_is_clinical_authority_but_not_patient_fact():
    derived = TruthRecord(
        key="tir_pct",
        value=68.2,
    ...
    assert derived.may_drive_clinical_decision is True
