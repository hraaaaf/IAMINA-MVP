"""Fail-closed manifest for live vision and OCR benchmark execution."""
...
class VisionProviderManifest:
    ...
    image_retention_confirmed: bool
    ...
    def validate(self, *, today: date) -> None:
        ...
        if missing:
            raise ValueError("missing vision benchmark manifest fields: " + ", ".join(missing))
        ...
        required_modalities = {"document_ocr", "glucometer_ocr", "meal_vision"}
        if not required_modalities.issubset(set(self.approved_modalities)):
            raise ValueError("provider is not approved for every vision benchmark modality")
        approvals = (
            self.no_training_confirmed,
            self.retention_confirmed,
            self.residency_confirmed,
            self.subprocessors_confirmed,
            self.image_retention_confirmed,
            self.approved_for_synthetic_benchmark,
        )
        if not all(approvals):
            raise ValueError("provider is not eligible for live vision benchmarking")
