"""One bounded live synthetic multi-turn conversation through IAMINA's real Companion path.

This benchmark intentionally retains the transcript because every user turn is
controlled synthetic text. It must never be repurposed for patient data.
"""
...
DATASET_ID = "iamina-live-synthetic-conversation-v1"
...
class LiveCompanionLLM:
    """Real Groq adapter, with provider usage captured per successful LLM turn."""
...
def validate_scenario() -> dict[str, int]:
    turns = scenario()
    if len(turns) != 10:
        raise RuntimeError("live conversation scenario must remain exactly 10 turns")
    if any(not turn.message.strip() for turn in turns):
        raise RuntimeError("scenario contains an empty message")
    routes = [resolved_route(turn) for turn in turns]
    counts = {name: routes.count(name) for name in ("safety", "zero_model", "llm")}
    if counts["safety"] < 2 or counts["zero_model"] < 2 or counts["llm"] < 5:
        raise RuntimeError(f"scenario route coverage too weak: {counts}")
    forbidden = ("@", "patient_id", "first_name", "last_name", "telephone", "email")
    lowered = "\n".join(turn.message.lower() for turn in turns)
    if any(token in lowered for token in forbidden):
        raise RuntimeError("scenario contains a forbidden identifier-shaped token")
    return counts
...
def run(output_path: Path) -> dict:
    ...
    ProviderBenchmarkPreflight(
        ...
        pricing_evidence_reference="docs/assessments/2026-08-25-frug9-prepilot-scenario-envelope.md",
        network_authorized=os.environ.get("IAMINA_CONVERSATION_NETWORK_AUTHORIZED", "").lower() == "true",
        spend_ceiling_microusd=SPEND_CEILING_MICROUSD,
        patient_data=False,
    ).validate()

    route_counts = validate_scenario()
    ...
    if llm.invalid_response:
        raise RuntimeError("provider returned an invalid Companion JSON response")
    if len(routes) != len(scenario()):
        raise RuntimeError("route capture does not match turn count")
    if {name: routes.count(name) for name in route_counts} != route_counts:
        raise RuntimeError("runtime routes diverged from preflight route classification")
    if len(llm.usage) != route_counts["llm"]:
        raise RuntimeError(
            f"expected {route_counts['llm']} successful LLM calls, got {len(llm.usage)}"
        )
    ...
    report = {
        ...
        "proof_boundaries": {
            ...
            "reason": "Real IAMINA Companion routing and real provider calls over controlled synthetic content only.",
        },
    }
...
def main() -> int:
    ...
    counts = validate_scenario()
    if args.validate_only:
        print(json.dumps({"valid": True, "route_counts": counts}, sort_keys=True))
        return 0
