...
used before real billing data exists, but a total monthly cost is only emitted when
all required usage and price inputs for that total are present.
"""
...
def build_scale_scaffold(
    ...
    """Build the four canonical FRUG-9 tiers without inventing missing inputs."""
    reports = tuple(_build_tier(mau, usage, prices) for mau in SCALE_TIERS)
...
def _build_tier(
    ...
    total = sum(components, Decimal(0)) if all(v is not None for v in components) else None
