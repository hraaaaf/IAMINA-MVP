...
TARGETS = {
    ...
    "AE": (
        "ebubekr53/organic-gulf-arabic-dialect-dataset",
...
def _source_snapshot(repo_id: str) -> SourceSnapshot:
    ...
    if license_name != EXPECTED_LICENSE:
        raise BenchmarkConfigurationError(
            f"{repo_id}: expected {EXPECTED_LICENSE}, "
            f"got {license_name or 'missing'}"
        )
    if not csv_paths:
...
def projected_spend_microusd(
    ...
    for case in cases:
        total += price.worst_case_microusd(
            input_tokens=len(
                (SYSTEM_PROMPT + _case_prompt(case)).encode("utf-8")
            ),
...
def run_benchmark(*, output_path: Path, today: date) -> dict[str, Any]:
    if not os.environ.get("GROQ_API_KEY", "").strip():
        raise BenchmarkConfigurationError(
            "missing GROQ_API_KEY benchmark credential"
        )

    unique_repos = sorted({repo_id for repo_id, _ in TARGETS.values()})
    snapshots = {
        repo_id: _source_snapshot(repo_id)
    ...
    if projected > SPEND_CEILING_MICROUSD:
        raise BenchmarkConfigurationError(
            "projected spend exceeds explicit ceiling"
        )
    ...
    for code in TARGETS:
        country_rows = [
            row
            for row in rows
            if row["expected_country_code"] == code
        ]
        by_country[code] = {
...
def main() -> int:
    ...
    per_country_ok = all(
        value["correct"] >= 2
        for value in summary["by_country"].values()
    )
